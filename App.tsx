...
const STORAGE_KEY = 'guardian_config_v24';
const LOGS_KEY = 'guardian_logs_v24';
...
const App: React.FC = () => {
  ...
  const [config, setConfig] = useState<SystemConfig>(() => {
    ...
    return saved ? JSON.parse(saved) : {
      ...
      customApiKey: '',
      customModel: '',
      contacts: [{ id: '1', name: '紧急呼助中心', phone: '120', relation: '公共服务', isPrimary: true }],
      ttsRate: 1.0,
      voiceType: 'ai',
      aiVoiceName: 'Kore',
      customTtsUrl: '',
  ...
  const handleSaveContact = () => {
    ...
    if (!editingContact.name || !editingContact.phone) return alert("请填写完整信息");
  ...
  const downloadCSV = () => {
    ...
    const header = ["编号", "时间", "事件", "类型", "级别", "备注"];
    const rows = logs.map(l => [l.id, l.timestamp.toLocaleString(), l.event, l.type, l.status, l.note || ""]);
    const csvContent = "\uFEFF" + [header, ...rows].map(r => r.join(",")).join("\n");
    const blob = new Blob([csvContent], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.setAttribute("href", url);
    link.setAttribute("download", `安全日志_${new Date().toLocaleDateString()}.csv`);
    link.click();
  };
  ...
  const handleFrameAnalysis = useCallback(async (base64: string) => {
    ...
    try {
      ...
      if (result.isFallDetected && result.confidence > 0.4) {
        ...
        dispatchSpeak(`警报！监测到意外情况，请问您需要帮助吗？`, config);
        setCountdown(10);
  ...
  useEffect(() => {
    ...
    if (status === SystemStatus.ALERT && countdown > 0) {
      ...
      dispatchSpeak(`确认险情。正在通知紧急联系人，请保持冷静。`, config);
      addLog(`执行紧急救援联络程序`, 'contact', 'danger');
    }
  ...
  return (
    <div className="flex h-screen bg-[#050505] text-zinc-300 font-sans overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto p-8 lg:p-12 custom-scrollbar relative">
            ...
            <header className="flex justify-between items-center">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest">状态: {status}</span>
              ...
              <button onClick={() => setStatus(status === SystemStatus.IDLE ? SystemStatus.MONITORING : SystemStatus.IDLE)} 
                ...
                {status === SystemStatus.IDLE ? '开启布防' : '撤防系统'}
              </button>
            </header>
            ...
            <div className="grid grid-cols-12 gap-8">
              <div className="col-span-12 xl:col-span-8 space-y-8">
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <StatsCard label="首选联络人" value={config.contacts.find(c => c.isPrimary)?.name || '未设置'} icon="fa-phone" color="indigo" />
                </div>
              </div>
          ...
          <div className="max-w-5xl mx-auto space-y-8 animate-in slide-in-from-bottom-4">
            <header className="flex justify-between items-end">
              <div>
                ...
                <p className="text-zinc-500 text-[10px] font-black mt-2 tracking-widest uppercase">存档记录: {logs.length} / 500</p>
              </div>
              <div className="flex gap-3">
                <button onClick={downloadCSV} className="px-6 py-2 bg-zinc-800 text-zinc-300 rounded-xl text-[10px] font-black uppercase border border-white/5 hover:bg-zinc-700 transition-all flex items-center gap-2">
                  <i className="fas fa-file-csv"></i> 导出 CSV
                </button>
                <div className="flex gap-2 bg-zinc-900 p-1.5 rounded-xl border border-white/5">
                  ...
                  <button onClick={() => setHistoryFilter('danger')} className={`px-6 py-2 rounded-lg text-[10px] font-black uppercase transition-all ${historyFilter === 'danger' ? 'bg-rose-600 text-white shadow-lg' : 'text-zinc-500'}`}>仅风险</button>
                </div>
              </div>
            </header>

            <div className="bg-zinc-900/40 border border-white/5 rounded-[2.5rem] overflow-hidden shadow-2xl backdrop-blur-3xl">
                  ...
                  <div key={log.id} className="p-8 border-b border-white/5 hover:bg-white/[0.02] flex items-center gap-8 group">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-1">
                        ...
                        {log.status === 'danger' && <span className="bg-rose-600 text-white text-[8px] px-2 py-0.5 rounded font-black uppercase tracking-tighter">异常事件</span>}
                      </div>
                    ...
                    <button onClick={() => setEditingLog(log)} className="opacity-0 group-hover:opacity-100 transition-opacity px-6 py-3 bg-white/5 border border-white/10 rounded-xl text-[10px] font-black uppercase text-indigo-400 hover:bg-indigo-600 hover:text-white">编辑备注</button>
                  </div>
                ))}
                {logs.length === 0 && <div className="py-40 text-center text-zinc-700 uppercase font-black tracking-widest text-xs">暂无历史记录</div>}
              </div>
            </div>
          </div>
        )}
        ...
        {activeTab === 'settings' && (
          <div className="max-w-5xl mx-auto space-y-12 animate-in slide-in-from-bottom-4 pb-32">
            <h2 className="text-4xl font-black text-white italic tracking-tighter uppercase mb-12 flex items-center gap-4">
              <i className="fas fa-sliders-h text-indigo-600"></i> 系统深度控制
            </h2>
            ...
            <section className="bg-gradient-to-br from-indigo-600/20 to-zinc-900 border border-white/10 p-10 rounded-[3rem] shadow-2xl relative overflow-hidden">
               <div className="absolute top-0 right-0 p-12 text-white/5 text-[15rem] pointer-events-none transform translate-x-1/4 -translate-y-1/4">
                 <i className="fas fa-cloud-download-alt"></i>
               </div>
              <h3 className="text-xl font-black text-white italic mb-8 uppercase tracking-widest flex items-center gap-4 border-l-4 border-indigo-600 pl-6">
                跨平台安装与部署指南
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 relative z-10">
                <DeployCard icon="fa-windows" title="PC / Windows" desc="在 Edge 或 Chrome 浏览器中点击地址栏右侧的“安装”图标，即可将本系统作为桌面 App 运行。" hint="支持任务栏固定与独立窗口" />
                <DeployCard icon="fa-apple" title="iPhone / iOS" desc="在 Safari 浏览器中点击底部的“分享”按钮，选择“添加到主屏幕”，即可像原生 App 一样开启。" hint="支持全屏无边框显示" />
                <DeployCard icon="fa-android" title="Android / 鸿蒙" desc="在 Chrome 浏览器中点击菜单，选择“安装应用”。系统会自动创建桌面快捷图标。" hint="支持高性能后台运行" />
              </div>
            </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              ...
              <section className="bg-zinc-900/40 border border-white/5 p-10 rounded-[3rem] shadow-xl backdrop-blur-md">
                <h3 className="text-lg font-black text-white italic mb-8 uppercase tracking-widest border-l-4 border-emerald-600 pl-4">推理引擎配置</h3>
                <div className="grid grid-cols-3 gap-2 p-1 bg-black rounded-xl border border-white/5 mb-8">
                  {[InferenceMode.CLOUD, InferenceMode.LOCAL, InferenceMode.CUSTOM].map(m => (
                    <button key={m} onClick={() => setConfig({...config, mode: m})} className={`py-3 rounded-lg text-[10px] font-black uppercase transition-all ${config.mode === m ? 'bg-emerald-600 text-white shadow-lg' : 'text-zinc-600'}`}>{m}</button>
                  ))}
                </div>
                  ...
                  {config.mode === InferenceMode.CLOUD && (
                    <div className="p-6 bg-emerald-500/5 border border-emerald-500/10 rounded-2xl">
                      <p className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                        <i className="fas fa-shield-check"></i> 云端托管加速中
                      </p>
                      <p className="text-xs text-zinc-500 leading-relaxed">当前由云端 Gemini 提供视觉分析，API Key 已通过环境变量注入，无需额外配置即可使用。</p>
                    </div>
                  )}

                  {config.mode === InferenceMode.LOCAL && (
                    <div className="space-y-4">
                      <InputGroup label="本地 Ollama 地址" value={config.localEndpoint} onChange={v => setConfig({...config, localEndpoint: v})} />
                      <InputGroup label="模型名称" value={config.localModel} onChange={v => setConfig({...config, localModel: v})} />
                    </div>
                  )}
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-2 mb-4">
                        {Object.entries(PRESETS).map(([key, p]) => (
                          <button key={key} onClick={() => setConfig({...config, customBaseUrl: p.url, customModel: p.model})} className="py-2.5 bg-zinc-800 border border-white/5 rounded-lg text-[9px] font-black uppercase text-zinc-400 hover:text-white">{p.name} 预设</button>
                        ))}
                      </div>
                      <InputGroup label="API 基准地址" value={config.customBaseUrl} onChange={v => setConfig({...config, customBaseUrl: v})} />
                      <InputGroup label="API 访问密钥" value={config.customApiKey} type="password" onChange={v => setConfig({...config, customApiKey: v})} />
                      <InputGroup label="模型 ID" value={config.customModel} onChange={v => setConfig({...config, customModel: v})} />
                    </div>
                  )}
                </div>
              </section>
              ...
              <section className="bg-zinc-900/40 border border-white/5 p-10 rounded-[3rem] shadow-xl flex flex-col justify-between backdrop-blur-md">
                <div>
                  <h3 className="text-lg font-black text-white italic mb-8 uppercase tracking-widest border-l-4 border-amber-600 pl-4">数据同步与备份</h3>
                  <div className="space-y-5">
                    <p className="text-xs text-zinc-500 leading-relaxed">您可以将当前配置（含 API 及联系人）导出为 JSON 文件，并在新电脑上一键恢复。</p>
                    <div className="grid grid-cols-2 gap-4">
                      <button onClick={exportConfig} className="py-5 bg-zinc-800 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 hover:bg-zinc-700 transition-all border border-white/5">
                        <i className="fas fa-file-export text-amber-500"></i> 导出配置文件
                      </button>
                      <button onClick={() => fileInputRef.current?.click()} className="py-5 bg-zinc-800 text-white rounded-2xl font-black uppercase text-[10px] tracking-widest flex items-center justify-center gap-3 hover:bg-zinc-700 transition-all border border-white/5">
                        <i className="fas fa-file-import text-indigo-500"></i> 导入配置文件
                      </button>
                    </div>
                ...
                <div className="mt-8 pt-8 border-t border-white/5">
                   <div className="flex justify-between items-center text-[10px] font-black mb-4">
                     <span className="text-zinc-500 uppercase">播报语速: {config.ttsRate.toFixed(1)}x</span>
                   </div>
                   <input type="range" min="0.5" max="2" step="0.1" value={config.ttsRate} onChange={e => setConfig({...config, ttsRate: parseFloat(e.target.value)})} className="w-full accent-indigo-600 h-1.5 bg-zinc-800 rounded-lg appearance-none cursor-pointer" />
                </div>
              </section>
              ...
              <section className="bg-zinc-900/40 border border-white/5 p-10 rounded-[3rem] shadow-xl backdrop-blur-md lg:col-span-2">
                <div className="flex justify-between items-center mb-10">
                  ...
                  <button onClick={() => setEditingContact({ id: Date.now().toString(), name: '', phone: '', relation: '', isPrimary: false })} 
                    ...
                    <i className="fas fa-plus"></i> 新增救护人
                  </button>
                </div>
                        ...
                        <div className="flex gap-4 items-center">
                          ...
                          <div>
                            <p className="text-sm font-black text-white flex items-center gap-2 italic">
                              {c.name} {c.isPrimary && <span className="bg-rose-600 text-[8px] px-1.5 py-0.5 rounded font-black uppercase">首选</span>}
                            </p>
                            <p className="text-[10px] text-zinc-500 font-mono mt-1 tracking-wider uppercase">{c.phone} · {c.relation}</p>
                          </div>
                        </div>
                        <div className="flex gap-2">
                          <button onClick={() => setEditingContact(c)} className="w-10 h-10 rounded-xl bg-zinc-900 text-zinc-500 hover:text-indigo-400 flex items-center justify-center border border-white/5"><i className="fas fa-pen text-[10px]"></i></button>
                          <button onClick={() => handleRemoveContact(c.id)} className="w-10 h-10 rounded-xl bg-zinc-900 text-zinc-500 hover:text-rose-500 flex items-center justify-center border border-white/5"><i className="fas fa-trash text-[10px]"></i></button>
                        </div>
                      </div>
                    </div>
                  ))}
                  {config.contacts.length === 0 && <div className="md:col-span-2 py-10 text-center text-zinc-600 border border-dashed border-white/10 rounded-3xl">暂未录入任何救护人员，警报时将无法自动呼救。</div>}
                </div>
              </section>
            </div>
          </div>
        ...
        {/* 联络人弹窗 */}
        {editingContact && (
          <div className="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[200] flex items-center justify-center p-6 animate-in fade-in duration-300">
            <div className="bg-zinc-950 border border-white/10 p-10 rounded-[3rem] w-full max-w-md shadow-2xl">
              <h3 className="text-2xl font-black italic text-white uppercase mb-8">联络人档案录入</h3>
              <div className="space-y-6">
                <InputGroup label="全名" value={editingContact.name} onChange={v => setEditingContact({...editingContact, name: v})} />
                <InputGroup label="手机号码 / 120" value={editingContact.phone} onChange={v => setEditingContact({...editingContact, phone: v})} />
                <InputGroup label="身份/关系" value={editingContact.relation} onChange={v => setEditingContact({...editingContact, relation: v})} />
                <div className="flex items-center gap-4 p-5 bg-white/5 rounded-2xl border border-white/5 cursor-pointer" onClick={() => setEditingContact({...editingContact, isPrimary: !editingContact.isPrimary})}>
                  <input type="checkbox" checked={editingContact.isPrimary} readOnly className="w-6 h-6 rounded accent-indigo-600" />
                  <label className="text-xs font-black text-zinc-400 uppercase tracking-widest cursor-pointer">标记为首要救护人</label>
                </div>
              </div>
              <div className="flex gap-4 mt-12">
                <button onClick={() => setEditingContact(null)} className="flex-1 py-5 bg-zinc-900 rounded-2xl text-[10px] font-black uppercase text-zinc-500">放弃</button>
                <button onClick={handleSaveContact} className="flex-1 py-5 bg-indigo-600 rounded-2xl text-[10px] font-black uppercase text-white shadow-xl shadow-indigo-900/20 active:scale-95 transition-all">保存档案</button>
              </div>
            </div>
          </div>
        )}
        ...
        {editingLog && (
          <div className="fixed inset-0 bg-black/95 backdrop-blur-3xl z-[200] flex items-center justify-center p-6 animate-in fade-in">
            <div className="bg-zinc-950 border border-white/10 p-10 rounded-[3rem] w-full max-w-lg shadow-2xl">
              <h3 className="text-2xl font-black italic text-white uppercase mb-8 tracking-tighter">添加历史备注</h3>
              <div className="space-y-6">
                <div className="p-5 bg-indigo-600/10 border border-indigo-600/20 rounded-2xl mb-4 text-xs font-bold">
                  事件描述: {editingLog.event}
                </div>
                <textarea className="w-full h-40 bg-black border border-white/10 p-6 rounded-3xl text-sm text-zinc-200 outline-none focus:border-indigo-600 transition-colors" placeholder="在此记录核实结果或救治情况..." value={editingLog.note || ""} onChange={e => setEditingLog({...editingLog, note: e.target.value})} />
              </div>
              <div className="flex gap-4 mt-12">
                <button onClick={() => setEditingLog(null)} className="flex-1 py-5 bg-zinc-900 rounded-2xl text-[10px] font-black uppercase text-zinc-500">取消</button>
                <button onClick={() => handleUpdateLog(editingLog)} className="flex-1 py-5 bg-indigo-600 rounded-2xl text-[10px] font-black uppercase text-white shadow-xl shadow-indigo-900/20 active:scale-95 transition-all">保存备注</button>
              </div>
            </div>
          </div>
        )}
        ...
        {status === SystemStatus.ALERT && (
          <div className="fixed inset-0 bg-black/98 backdrop-blur-[100px] z-[300] flex items-center justify-center p-8 animate-in zoom-in-110 duration-500">
            <div className="max-w-2xl w-full text-center">
              <div className="text-[15rem] font-black text-white mb-6 leading-none tracking-tighter italic animate-pulse">{countdown}</div>
              <h2 className="text-4xl font-black text-rose-500 italic mb-12 uppercase tracking-tight">监测到跌倒风险 // 请确认</h2>
              <div className="grid grid-cols-2 gap-6">
                <button onClick={() => { stopSpeaking(); setStatus(SystemStatus.MONITORING); addLog("人工干预: 误报", "human", "info"); }} className="py-8 bg-zinc-900 text-zinc-500 rounded-[2.5rem] font-black uppercase tracking-[0.2em] text-xs hover:text-white transition-all border border-white/10">安全/误报</button>
                <button onClick={() => { stopSpeaking(); setStatus(SystemStatus.EMERGENCY); }} className="py-8 bg-rose-600 text-white rounded-[2.5rem] font-black uppercase shadow-2xl shadow-rose-600/40 tracking-[0.2em] text-xs active:scale-95 transition-all">确认险情</button>
              </div>
            </div>
          </div>
        )}
...
const NavTab: React.FC<{ active: boolean; onClick: () => void; icon: string; label: string }> = ({ active, onClick, icon, label }) => (
  <button onClick={onClick} className={`w-full flex items-center gap-5 p-5 rounded-2xl transition-all relative group ${
    ...
    {active && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1.5 h-10 bg-white rounded-full shadow-[0_0_15px_rgba(255,255,255,0.5)]"></div>}
  </button>
);
...
const DeployCard: React.FC<{ icon: string; title: string; desc: string; hint: string }> = ({ icon, title, desc, hint }) => (
  <div className="bg-black/40 border border-white/5 p-8 rounded-[2rem] hover:bg-black/60 transition-all group relative overflow-hidden h-full">
    <div className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center text-white mb-6 group-hover:bg-indigo-600 transition-all duration-300 shadow-xl">
    ...
    <div className="pt-5 border-t border-white/5 text-[8px] font-black text-indigo-400 uppercase">Pro 贴士: {hint}</div>
  </div>
);
